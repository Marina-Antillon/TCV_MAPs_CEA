...
for(scenario_switch in 1:3){
  load(paste0(tmp_subdir_in, '/wq_maps_NOunc_comp', scenario_switch, '.Rdata'))

  epipar = rep(NA,8)
  names(epipar) = lbl$par
  epipar[1:5] = c(exp(pars_unc[1]), exp(pars_unc[2]), 
  ...
  ggsave(paste0(tmp_subdir_fig, "/typh_cases_sanity.pdf"), 
  ...
  ggsave(paste0(tmp_subdir_fig, "/typh_cases_sanity_byWQ.pdf"), 
         width = 10, height = 10, units="in", dpi=600)
  ...
  ggsave(paste0(tmp_subdir_fig, "/doses_sanity_byWQ.pdf"), 
